...
const Visualizations = {
    ...
    initMaps() {
        // If a map already exists, unhook the move handler first, then remove the map
        ...
        const el = document.getElementById('mapVisualization');
        if (!el) {
            console.warn('#mapVisualization not found');
            return;
        }

        STATE.maps.incidents = L.map(el, {
            center: [35.2271, -80.8431],
            zoom: 11,
            zoomControl: true,
            preferCanvas: true // faster for many points
        });
        ...
        // Create (or reuse) a single Canvas renderer for all points
        STATE._canvasRenderer = STATE._canvasRenderer || L.canvas({padding: 0.5});

        // Ensure map sizes correctly after layout
        setTimeout(() => STATE.maps.incidents.invalidateSize(), 100);

        // Re-render points only after pans/zooms (guard against duplicate listeners)
        STATE._moveHandler = Utils.debounce(() => this.updateIncidentsMap(true), 150);
        STATE.maps.incidents.on('moveend zoomend', STATE._moveHandler);
    },
    ...
    updateIncidentsMap(onlyRedrawPoints = false) {
        ...
        if (!onlyRedrawPoints) {
            ...
            const countsByZip = data.reduce((acc, d) => {
        ...
        // redraw points (fast path)
        layers.crimePoints.clearLayers();

        const bounds = map.getBounds();
        const renderer = STATE._canvasRenderer || (STATE._canvasRenderer = L.canvas({padding: 0.5}));

        // ✅ set a zoom-based radius ONCE per redraw
        const zoom = map.getZoom();
        const r = Math.max(2, Math.min(6, (zoom - 9))); // 2..6 px roughly as you zoom

        for (const cr of data) {
            if (!cr.latitude || !cr.longitude) continue;

            const latlng = [cr.latitude, cr.longitude];
            if (!bounds.contains(latlng)) continue; // only draw visible points

            const marker = L.circleMarker(latlng, {
                renderer,
                radius: r,                 // ✅ uses zoom-scaled radius
                weight: 1,
                color: '#fff',
                fillColor: Utils.getCrimeColor(cr.category),
                fillOpacity: 0.9
            });

            marker.bindPopup(
                `<strong>${cr.type}</strong><br/>Date: ${Utils.formatDate(cr.date)}<br/>Category: ${cr.category}<br/>${cr.address ? 'Location: ' + cr.address : ''}`
            );

            layers.crimePoints.addLayer(marker);
        }

        if (!onlyRedrawPoints) this.updateMapLegend('mapLegend');
    },
    ...
    updateViolentStackedArea() {
        ...
        overlay
            .on('mousemove', event => {
                ...
                Utils.showTooltip(
                    event,
                    `<strong>${row.date.toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'short'
    ...
    renderMap(map, layers, data, zipCodes, legendId) {
        ...
        // Precompute counts per ZIP once to avoid O(n*m) filtering
        const countsByZip = {};
        data.forEach(crime => {
            const z = crime.zipCode;
            if (z) countsByZip[z] = (countsByZip[z] || 0) + 1;
        });
        ...
        if (zipCodes && zipCodes.length > 0) {
            zipCodes.forEach(zipFeature => {
                ...
                const crimeCount = countsByZip[zipCode] || 0;
                const style = {
                    fillColor: '#e0e0e0',
                    weight: 2,
                    opacity: 1,
                ...
                const layer = L.geoJSON(zipFeature, {
                    ...
                    onEachFeature: (feature, layer) => {
                        ...
                        layer.bindPopup(
                            `<strong>ZIP Code: ${zipCode}</strong><br/>Incidents: ${crimeCount}`
                        );
                    }
                });
        ...
        // Add crime points. Use circleMarker with Canvas renderer for better performance
        ...
        data.forEach(crime => {
            ...
            marker.bindPopup(
                `<strong>${crime.type}</strong><br/>Date: ${Utils.formatDate(crime.date)}<br/>Category: ${crime.category}<br/>${
                    crime.address ? 'Location: ' + crime.address : ''
                }`
            );
            // Add to crimePoints layer group if available, else directly to map
            if (layers && layers.crimePoints) {
                layers.crimePoints.addLayer(marker);
            } else {
                marker.addTo(map);
            }
